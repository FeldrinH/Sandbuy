...
local huddrawlist = { ["CHudHealth"] = true, ["CHudBattery"] = true, ["CHudAmmo"] = true, ["CHudSecondaryAmmo"] = true, ["CHudZoom"] = true, ["CHudSuitPower"] = true }
...
function SWEP:DrawHUD()
	...
	local scale = Vector( w/12*(1-2/9), w/12*(1-2/9), 0 )
	local hitpos = self.Owner:GetEyeTrace().HitPos
	local startpos = self.Owner:GetEyeTrace().StartPos
	...
	if self.Weapon:GetNWBool("Active", false) then
		...
		draw.SimpleText(self.Weapon:GetNWInt("ZoomMult").."x", "ScoreboardText", w/2-w/48, h/2-w/48, green, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
...
function SWEP:HUDShouldDraw( name )
	...
	if	self:GetNWBool("Active", true) then
		if huddrawlist[name] then return false end
	else
